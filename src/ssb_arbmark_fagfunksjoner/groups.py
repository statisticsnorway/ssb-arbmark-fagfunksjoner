...
def nace_sn07_47grp(nace_sn07: pd.Series, labels=True) -> pd.Series:
    ...
    # Define the conditions for each group
    conditions = [
        np.isin(nace2, ['01', '02', '03']),  # Jordbruk, skogbruk, fiske; Bergverksdrift og utvinning, utenom olje og gass
        np.logical_or(np.isin(nace2, ['05', '07', '08']), nace3 == '099'),  # Annen utvinning; Bygging av skip og båter; Reparasjon og installasjon av maskiner og utstyr; Uoppgitt utvinning
        np.logical_or(nace2 == '06', nace3 == '091'),  # Olje- og gassutvinning; Uoppgitt utvinning av petroleum
        ...
        np.isin(nace2, ['19', '20', '21']),  # Petrolieum, kull, kjemisk og farmasøytisk industri
        ...
        np.logical_or(np.isin(nace2, ['29', '33']), np.logical_and(nace3 >= '302', nace3 <= '309')),  # Transportmidelindustri, utenom 30.1; Produksjon av kjøretøy og tilhengere, unntatt motorvogner og motorsykler
        ...
        np.isin(nace2, ['73', '74', '75']),  # Faglig, vitenskapelig og teknisk tjenesteyting ellers
        np.logical_and(nace2 >= '77', nace2 <= '82'),  # Forretningsmessig tjenesteyting ellers
...
def nace_sn07_17grp(nace_sn07: pd.Series) -> pd.Series:
    """
    Categorize a pandas Series of NACE-codes (SN07) into predefined groups.
    ...
    if n_unique_grp > 48:
        print(f"Warning: There are {n_unique_grp} unique industry divisions on 2-number level. The function first groups the input into the 47 groups standard.")
        nace_str2 = nace_sn07_47grp(nace_sn07, labels=False)

    # Define the conditions for each group
    conditions = [
        nace_str2 == '01',                                     # 01-03 Jordbruk, skogbruk og fiske
        np.logical_and(nace_str2 >= '01', nace_str2 <= '03'),  # 05-09 Bergverksdrift og utvinning
        np.logical_and(nace_str2 >= '04', nace_str2 <= '16'),  # 10-33 Industri
        np.logical_and(nace_str2 >= '17', nace_str2 <= '18'),  # 35-39 Elektrisitet, vann og renovasjon
        nace_str2 == '19',                                     # 41-43 Bygge- og anleggsvirksomhet
        np.logical_and(nace_str2 >= '20', nace_str2 <= '22'),  # 45-47 Varehandel, reparasjon av motorvogner
        np.logical_and(nace_str2 >= '23', nace_str2 <= '27'),  # 49-53 Transport og lagring
        np.logical_and(nace_str2 >= '28', nace_str2 <= '29'),  # 55-56 Overnattings- og serveringsvirksomhet
        np.logical_and(nace_str2 >= '30', nace_str2 <= '31'),  # 58-63 Informasjon og kommunikasjon
        np.logical_and(nace_str2 >= '32', nace_str2 <= '34'),  # 64-66 Finansiering og forsikring
        np.logical_and(nace_str2 >= '35', nace_str2 <= '38'),  # 68-75 Teknisk tjenesteyting, eiendomsdrift
        nace_str2 == '39',                                     # 77-82 Forretningsmessig tjenesteyting
        nace_str2 == '40',                                     # 84 Off.adm., forsvar, sosialforsikring
        nace_str2 == '41',                                     # 85 Undervisning
        np.logical_and(nace_str2 >= '42', nace_str2 <= '43'),  # 86-88 Helse- og sosialtjenester
        np.logical_and(nace_str2 >= '44', nace_str2 <= '47')   # 90-99 Personlig tjenesteyting
    ]
...
def sektor2_grp(sektor: pd.Series, undersektor: pd.Series, labels=True) -> pd.Series:
    ...
    conditions = [
        ...
        ansatte >= 250,                                  # 250 employees or more
    ]
