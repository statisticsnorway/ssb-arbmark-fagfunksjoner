...
def sample_df() -> pd.DataFrame:
    return pd.DataFrame(
        {
            "antall_ansatte": pd.to_numeric(
                [np.nan, None, 0, 1.00001, 975, 25]
            ),
...
def test_sb_integer(sample_df: pd.DataFrame) -> None:
    test1_result = sb_integer(
        sample_df["antall_ansatte"], unit=2
    ).to_list()
    ...
    test2_result = sb_integer(
        sample_df["antall_ansatte"], unit=1
    ).to_list()
    ...
    test3_result = sb_integer(
        sample_df["antall_ansatte"], unit=0
    ).to_list()
...
def test_sb_percent(sample_df: pd.DataFrame) -> None:
    test1_result = sb_percent(
        sample_df["sykefravaersprosent"], decimals=1
    ).to_list()
    ...
    test2_result = sb_percent(
        sample_df["sykefravaersprosent"], decimals=2
    ).to_list()
